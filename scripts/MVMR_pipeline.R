...
for (i in seq_len(exp_n)) {
  ...
  setnames(mvmr_input, c("betaX", "sebetaX"), c(paste0("betaX", i), paste0("sebetaX", i)))
...
setcolorder(mvmr_input, c("SNP", "variant_id", "rsid", "iv_sources", setdiff(names(mvmr_input), c("SNP", "variant_id", "rsid", "iv_sources"))))
write_txt(mvmr_input, file.path(result_dir, "mvmr_input.txt"))
cat("  MVMR 最终输入 SNP 数:", nrow(mvmr_input), "\n")
